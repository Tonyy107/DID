...
func _physics_process(delta):
	...
	move_and_slide()  # In Godot 4, no arguments are needed
...
func move_character(delta):
	if is_dashing:
		velocity.y = 0
		var dash_direction = input_direction if input_direction != 0 else 1
		velocity.x = DASH_SPEED * sign(dash_direction)
		return
...
func jump_logic():
	...
	# Variable jump height (if jump released early)
